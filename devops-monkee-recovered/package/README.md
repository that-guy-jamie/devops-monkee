...
DevOps Monkee (CLI: `dopm` or `sbep`) implements the **SBEP (Source-Bound Execution Protocol)** - a framework for governing AI agent behavior and ensuring development quality.
...
- **ABIDE Command**: One-command SBEP compliance checking
...
**Short Aliases**: The package installs three command aliases - use whichever you prefer:
- `devops-monkee` - Full name
- `sbep` - Protocol acronym  
- `dopm` - DevOps Monkee shorthand (DOpM)
...
### 0. ABIDE - Check Everything

The `abide` command is your "all-in-one" SBEP compliance checker. When in doubt, ABIDE:
...
**💡 ABIDE Philosophy**: When something's unclear, the answer is probably in SBEP documentation. The `abide` command helps you find it.
...
```bash
# Initialize governance for current project (use any alias: dopm, sbep, or devops-monkee)
dopm init .
# or: sbep init .
# or: devops-monkee init .

# Or specify a different path
dopm init /path/to/your/project
```
...
# Get detailed report
dopm validate . --report validation-report.json
...
```bash
# Full quality audit
dopm audit . --type comprehensive

# Security audit only
dopm audit . --type security

# Save results to file
dopm audit . --output audit-results.json
```
...
# Or use npx (with any alias: dopm, sbep, or devops-monkee)
npx dopm --help
# or: npx sbep --help
# or: npx devops-monkee --help
```
...
*DevOps Monkee (DOpM) - Governing AI agents, one protocol at a time.*  
*Run `dopm --help` to get started.*
