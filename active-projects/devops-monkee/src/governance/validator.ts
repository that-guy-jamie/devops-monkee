...
export class Validator {
  ...
  async validate(projectPath: string, options: any = {}): Promise<ValidationResult> {
    ...
    const grade = await this.calculateGrade(score);
  ...
  private async validateVersionConsistency(projectPath: string): Promise<{ score: number; issues: ValidationIssue[] }> {
    ...
    const versionManifest = await this.versionManager.loadManifest();
    const currentVersions = versionManifest.versions;
    ...
    for (const [component, version] of Object.entries(manifest.versions?.components || {})) {
      if (typeof version === 'object' && version && 'current' in version && typeof version.current === 'string') {
  ...
  private async validateQualityMetrics(projectPath: string): Promise<{ score: number; issues: ValidationIssue[] }> {
    ...
    // Get schema data
    const schema = await this.schemaManager.loadSchema();
    const metrics = schema.validation_rules.quality_metrics;
  ...
  private async calculateGrade(score: number): Promise<'A' | 'B' | 'C' | 'D' | 'F'> {
    const schema = await this.schemaManager.loadSchema();
