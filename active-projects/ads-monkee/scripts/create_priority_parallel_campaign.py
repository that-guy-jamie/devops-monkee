#!/usr/bin/env python3
"""
Create parallel campaign for Priority Roofing (correct account) with $100 budget.
"""
import sys
...
def get_priority_roofing_campaign(customer_id: str) -> tuple:
    ...
    # Find the main Priority Roofing campaign
...
def main():
    ...
    new_campaign_name = "Priority Roofing - Optimized Parallel"
    target_budget_micros = 100000000  # $100 daily budget (not $22)
    ...
    try:
        ...
        # Step 2: Create new campaign in PAUSED state with $100 budget
        resource_name = create_paused_campaign(
            customer_id=customer_id,
            campaign_name=new_campaign_name,
            daily_budget_micros=target_budget_micros
        )

        # Step 3: Validate the created campaign
        validation_pass = validate_campaign_budget(customer_id, resource_name, target_budget_micros)

        if validation_pass:
            print("\n[SUCCESS] PRIORITY ROOFING PARALLEL CAMPAIGN CREATION SUCCESSFUL!")
            ...
            print(f"Budget: $100.00/day (not $22)")
